...
// ===== Init =====// ===== API数据同步 =====var _apiAvailable = false;async function syncJobsFromAPI() {  try {    var resp = await API.getJobs({ pageSize: 2000 });    if (resp && resp.success && resp.data && resp.data.length > 0) {      if (typeof DataService !== "undefined" && DataService.importJobs) {        DataService.importJobs(resp.data);      }      _apiAvailable = true;      console.log("[API] 同步 " + resp.data.length + " 条岗位数据到DataService");      // 触发UI刷新      updateLiveStats();      updateLiveIndicator();      if (state.currentPage === "home") {        state.jobPage = 1;        if (state.currentView === "table") renderJobTable();        else renderJobCards();      }      return true;    }  } catch (e) {    console.log("[API] 后端未连接，使用本地数据");    _apiAvailable = false;  }  return false;}function getJobsData() {  if (typeof DataService !== "undefined") {    var data = DataService.getJobs();    if (data && data.length > 0) return data;  }  return window.mockJobs || [];}function init() {
  // 动态加载 data.js（避免 defer 阻塞 DOMContentLoaded）
  ...
  dataScript.onerror = function() {
    console.error("data.js 加载失败，尝试使用备用数据");
    _initCore();
  };
